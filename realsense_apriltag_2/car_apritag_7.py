...
import numpy as np
from ultralytics import YOLO  # SAHI 대신 YOLO 사용
import math
...
# [수정된 부분] YOLO 모델 및 클래스 설정
# model_path = '/home/limdoyeon/realsense_apriltag_2/runs/detect/EV_Plate_Master_v4/weights/best.pt'
model_path = '/home/limdoyeon/realsense_apriltag_1/runs/detect/color_yolo26x_sca03_deg40_mAP50945/weights/best.pt'
model = YOLO(model_path)
# TARGET_CLASSES = [0,1 , 12, 23, 41, 43, 44] # 0,2,3,5,7,8 숫자 인덱스
...
try:
    while True:
        ...
        # [수정된 부분] YOLO 추론 (SAHI 루프와 동일한 구조로 데이터 추출)
        results = model.predict(img_bgr, conf=CONF_THRESHOLD, verbose=False)[0]

        detections = []
        # YOLO의 결과를 기존 SAHI의 obj 구조처럼 순회합니다.
